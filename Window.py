...
class Window(QWidget):
    ...
    def initUI(self):
        ...
        names = [config.X, config.Y, config.Z, config.HADAMARD,config.S, config.S + config.HERM, config.T, config.T + config.HERM, config.SWAP, config.CONTROL, config.MEASUREMENT]
        buttons = []
        for name in names:
